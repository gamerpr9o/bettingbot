...
// ─── FIREBASE HELPERS (async, shared across all users) ────────────────────────
...
const saveStore = async (data) => {
  await set(ref(db, "accounts"), data);
};
...
const saveAccount = async (key, data) => {
  await set(ref(db, `accounts/${key}`), data);
};

const updateAccount = async (key, partial) => {
  await update(ref(db, `accounts/${key}`), partial);
};
...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    ...
    --glow2: 0 0 40px rgba(99,102,241,0.2);
  }
...
function Particle({ style }) {
  return (
    <div style={{
      ...
      background: `rgba(99,102,241,${Math.random() * 0.6 + 0.2})`,
      borderRadius: "50%",
...
function StatusPill({ connected }) {
  return (
    <div style={{
      ...
      padding: "6px 16px",
      background: connected ? "rgba(16,217,138,0.1)" : "rgba(244,63,94,0.1)",
      border: `1px solid ${connected ? "rgba(16,217,138,0.4)" : "rgba(244,63,94,0.4)"}`,
      borderRadius: 100,
      fontSize: 12, fontWeight: 600, letterSpacing: "0.08em",
      fontFamily: "'JetBrains Mono', monospace",
      color: connected ? "#10d98a" : "#f43f5e",
      animation: connected ? "borderGlow 2s ease-in-out infinite" : "none",
    }}>
      <span style={{
        width: 8, height: 8, borderRadius: "50%",
        background: connected ? "#10d98a" : "#f43f5e",
        animation: connected ? "pulse 1.5s ease-in-out infinite" : "none",
        boxShadow: connected ? "0 0 8px #10d98a" : "none",
      }} />
...
function StatCard({ label, value, sub, color = "#6366f1", icon, delay = 0, prefix = "", suffix = "" }) {
  ...
  return (
    ...
    onMouseLeave={e => {
      ...
      <div style={{ fontSize: 11, color: "var(--text3)", letterSpacing: "0.1em", marginBottom: 8, fontWeight: 600 }}>
        {label}
      </div>
      <div style={{
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      <input type="range" min={min} max={max} step={step} value={value}
        ...
        style={{
          ...
          background: `linear-gradient(90deg, ${color} ${((value-min)/(max-min))*100}%, var(--surface2) 0%)`,
          cursor: "pointer",
        }} />
    </div>
  );
...
function Input({ label, value, onChange, type = "text", placeholder = "", readonly = false, mono = false }) {
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      <input
        ...
        style={{
          width: "100%",
          background: readonly ? "var(--bg2)" : "var(--surface)",
          border: "1px solid var(--border)",
          borderRadius: 10,
          padding: "10px 14px",
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1px solid ${variant === "primary" ? "transparent" : "currentColor"}`,
...
function Ticker({ account }) {
  const items = account ? [
    `💰 Balance: $${account.stats.balance.toFixed(2)}`,
    `📈 Today: ${account.stats.todayProfit >= 0 ? "+" : ""}$${account.stats.todayProfit.toFixed(2)}`,
    `🎯 Win Rate: ${account.stats.winRate.toFixed(1)}%`,
    `📊 Total Bets: ${account.stats.totalBets}`,
    `🔥 Streak: ${account.stats.streak}`,
    `💹 ROI: ${account.stats.roi.toFixed(1)}%`,
    `⚡ Open Bets: ${account.stats.openBets}`,
    `🏆 Best Day: +$${account.stats.bestDay.toFixed(2)}`,
...
function ReportRow({ r }) {
  ...
  return (
    <div style={{
      display: "grid", gridTemplateColumns: "120px 1fr 1fr 1fr 1fr 1fr",
    ...
    onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
      ...
      <span style={{ color: pos ? "#10d98a" : "#f43f5e", fontWeight: 700 }}>
...
function KeyScreen({ onEnter }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    setErr("");
    const acc = await loadAccount(key.trim());
    if (acc) {
      onEnter(key.trim(), acc);
    } else {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow: "0 40px 80px rgba(0,0,0,0.5), var(--glow2)",
      }}>
        {/* Logo */}
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <div style={{
            ...
            boxShadow: "var(--glow), 0 8px 20px rgba(0,0,0,0.4)",
        ...
        <div style={{
          ...
          border: "1px solid var(--border)",
          fontSize: 11, color: "var(--text3)", textAlign: "center", lineHeight: 1.6,
        }}>
          Keys are provided by the administrator.<br/>
          Each key is linked to a single account and website.
        </div>
      </div>
    </div>
  );
...
function SetupScreen({ keyVal, onComplete }) {
  ...
  const handleSetup = async () => {
    ...
    setLoading(true);
    const existing = await loadAccount(keyVal);
    const acc = { ...existing, username: username.trim(), website: website.trim() };
    await saveAccount(keyVal, acc);
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <p style={{ color: "var(--text3)", fontSize: 13, marginTop: 6 }}>
            Set up your account. This can only be done once.
          </p>
        </div>
        ...
        <Input label="BETTING WEBSITE" value={website} onChange={setWebsite} placeholder="e.g. Bet365, Betfair..." />

        {err && <div style={{ color: "var(--red)", fontSize: 12, marginBottom: 12 }}>{err}</div>}

        <Btn onClick={handleSetup} disabled={loading || !username || !website} fullWidth>
...
function AdminPanel({ onClose }) {
  ...
  const [store, setStore_] = useState({});
  const [newKey, setNewKey] = useState("");
  const [msg, setMsg] = useState("");
  const [msgType, setMsgType] = useState("success");

  useEffect(() => {
    loadStore().then(setStore_);
    // Live updates: re-fetch every 5s
    const interval = setInterval(() => loadStore().then(setStore_), 5000);
  ...
  const refresh = () => loadStore().then(setStore_);
  ...
  const genKey = () => {
    const k = Array.from({length:4}, () => Math.random().toString(36).substr(2,4).toUpperCase()).join("-");
    setNewKey(k);
  };

  const saveKey = async () => {
    if (!newKey) return;
    await saveAccount(newKey, emptyAccount(newKey));
    await refresh();
  ...
  const addReport = async () => {
    ...
    const acc = { ...s[repKey] };
    acc.dailyReports = [{ ...report }, ...(acc.dailyReports || [])];
    acc.stats = { ...acc.stats };
    acc.stats.todayProfit = report.profit;
    acc.stats.totalProfit += report.profit;
    acc.stats.totalBets += report.bets;
    acc.stats.wonBets += Math.round(report.bets * report.winRate / 100);
    acc.stats.lostBets += Math.round(report.bets * (1 - report.winRate / 100));
    acc.stats.balance += report.profit;
    acc.stats.volume += report.volume;
    if (report.profit > acc.stats.bestDay) acc.stats.bestDay = report.profit;
    if (report.profit < acc.stats.worstDay) acc.stats.worstDay = report.profit;
    acc.stats.winRate = acc.stats.totalBets > 0 ? (acc.stats.wonBets / acc.stats.totalBets) * 100 : report.winRate;
    acc.stats.roi = acc.stats.volume > 0 ? (acc.stats.totalProfit / acc.stats.volume) * 100 : 0;
    await saveAccount(repKey, acc);
    await refresh();
  ...
  const deleteReport = async (key, idx) => {
    const s = await loadStore();
    const acc = { ...s[key], dailyReports: [...(s[key].dailyReports || [])] };
    acc.dailyReports.splice(idx, 1);
  ...
  const loadEditStats = async (key) => {
    setEditKey(key);
    if (!key) return;
    const acc = await loadAccount(key);
    if (acc) setEditStats({ ...acc.stats });
  };

  const saveEditStats = async () => {
    if (!editKey || !editStats) return;
    await update(ref(db, `accounts/${editKey}/stats`), editStats);
    await refresh();
    flash("Stats saved for " + (store[editKey]?.username || editKey) + " ✓");
  };

  const setStat = (k, v) => setEditStats(prev => ({ ...prev, [k]: Number(v) }));

  const handleDiscordCommand = async () => {
    if (!discordKey) return flash("Enter a key", "warn");
    const acc = await loadAccount(discordKey);
    if (!acc) return flash("Key not found", "warn");
    await update(ref(db, `accounts/${discordKey}`), { connected: discordAction === "running" });
    await refresh();
  ...
  // Stat fields definition for the editor
  const statFields = [
    { key: "balance",       label: "Balance ($)",         prefix: "$" },
    { key: "todayProfit",   label: "Today's Profit ($)",  prefix: "$" },
    { key: "totalProfit",   label: "Total Profit ($)",    prefix: "$" },
    { key: "totalBets",     label: "Total Bets",          prefix: ""  },
    { key: "wonBets",       label: "Won Bets",            prefix: ""  },
    { key: "lostBets",      label: "Lost Bets",           prefix: ""  },
    { key: "winRate",       label: "Win Rate (%)",        prefix: ""  },
    { key: "avgOdds",       label: "Avg Odds",            prefix: ""  },
    { key: "streak",        label: "Current Streak",      prefix: ""  },
    { key: "bestDay",       label: "Best Day ($)",        prefix: "$" },
    { key: "worstDay",      label: "Worst Day ($)",       prefix: "$" },
    { key: "roi",           label: "ROI (%)",             prefix: ""  },
    { key: "volume",        label: "Volume ($)",          prefix: "$" },
    { key: "openBets",      label: "Open Bets",           prefix: ""  },
    { key: "pendingProfit", label: "Pending Profit ($)",  prefix: "$" },
  ];
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: 11, color: "var(--text3)", fontFamily: "monospace", marginLeft: 8 }}>
              {accounts.length} keys · {setupAccounts.length} active
            </span>
          </div>
        ...
        <div style={{ display: "flex", gap: 6, padding: "10px 24px", borderBottom: "1px solid var(--border)", flexShrink: 0, flexWrap: "wrap" }}>
          {[
            ...
            { id: "stats",   label: "Edit Stats",  icon: "✏️" },
            { id: "reports", label: "Daily Reports",icon: "📊" },
            { id: "discord", label: "Discord Cmd", icon: "🤖" },
        ...
        {msg && (
          <div style={{
            ...
            background: msgType === "warn" ? "rgba(245,158,11,0.1)" : "rgba(16,217,138,0.1)",
            borderBottom: `1px solid ${msgType === "warn" ? "rgba(245,158,11,0.25)" : "rgba(16,217,138,0.2)"}`,
            color: msgType === "warn" ? "var(--orange)" : "#10d98a",
          }}>
            {msgType === "warn" ? "⚠️" : "✓"} {msg}
          </div>
        )}
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 10, marginBottom: 28 }}>
                <input value={newKey} onChange={e => setNewKey(e.target.value)}
                  placeholder="Click Generate or type custom key"
                  style={{
              ...
              {accounts.length === 0 && (
                <div style={{ color: "var(--text3)", fontSize: 13, textAlign: "center", padding: 40 }}>No keys yet. Generate one above.</div>
              )}
              {accounts.map(acc => (
                <div key={acc.key} style={{
                  display: "flex", alignItems: "center", justifyContent: "space-between",
                  padding: "12px 16px", borderRadius: 12,
                  border: "1px solid var(--border)", marginBottom: 8, background: "var(--surface)",
                }}>
                  <div>
                    <span style={{ fontFamily: "monospace", color: "var(--accent)", fontWeight: 700, fontSize: 14, marginRight: 14 }}>{acc.key}</span>
                    <span style={{ fontSize: 12, color: "var(--text3)" }}>
                      {acc.username ? `${acc.username} · ${acc.website}` : "⏳ Not setup yet"}
                    </span>
                  </div>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <span style={{ fontSize: 11, fontFamily: "monospace", color: acc.connected ? "#10d98a" : "var(--text3)" }}>
                      {acc.connected ? "● LIVE" : "○ OFFLINE"}
                    </span>
                    {acc.username && (
                      <span style={{ fontSize: 11, color: "var(--text3)", fontFamily: "monospace" }}>
                        ${acc.stats.balance.toFixed(0)}
                      </span>
                    )}
          ...
          {tab === "stats" && (
            <div>
              ...
              <p style={{ fontSize: 12, color: "var(--text3)", marginBottom: 18, lineHeight: 1.6 }}>
                Select a user, edit any value, then click Save. Changes appear instantly on their dashboard.
              </p>

              {/* User selector */}
              <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
                <select value={editKey} onChange={e => loadEditStats(e.target.value)}
                  style={{
                    flex: 1, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10,
              ...
              {!editKey && (
                <div style={{ textAlign: "center", padding: 40, color: "var(--text3)", fontSize: 13 }}>
                  Select a user above to edit their stats
                </div>
              )}
              ...
              {editKey && editStats && (
                <>
                  ...
                  <div style={{
                    ...
                    {[
                      ["User", setupAccounts.find(a => a.key === editKey)?.username],
                      ["Site", setupAccounts.find(a => a.key === editKey)?.website],
                      ["Key", editKey],
                    ].map(([l, v]) => (
                      <span key={l} style={{ color: "var(--text3)" }}>
                        {l}: <b style={{ color: "var(--text)", fontFamily: "monospace" }}>{v}</b>
                      </span>
                    ))}
                  </div>
          ...
          {tab === "reports" && (
            <div>
              ...
              <p style={{ fontSize: 12, color: "var(--text3)", marginBottom: 16 }}>
                Adding a report automatically updates the user's cumulative stats (balance, total profit, bets, win rate, ROI, best/worst day).
              </p>

              <Select label="SELECT USER" value={repKey} onChange={setRepKey}
                options={[{ id: "", label: "-- Select a user --" }, ...setupAccounts.map(a => ({ id: a.key, label: `${a.username} (${a.key})` }))]} />

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 14 }}>
                {[
                  { label: "DATE", key: "date", type: "date" },
                  { label: "PROFIT / LOSS ($)", key: "profit", type: "number" },
                  { label: "TOTAL BETS", key: "bets", type: "number" },
                  { label: "WIN RATE (%)", key: "winRate", type: "number" },
                  ...
                  <div key={key}>
                    <div style={{ fontSize: 10, color: "var(--text3)", letterSpacing: "0.08em", marginBottom: 5, fontWeight: 600 }}>{label}</div>
                    <input
                      type={type} value={report[key]}
                      onChange={e => setReport(r => ({ ...r, [key]: type === "number" ? Number(e.target.value) : e.target.value }))}
                      placeholder={key === "site" ? "Bet365" : ""}
                      style={{
              ...
              {/* Existing reports for selected user */}
              {repKey && (() => {
                const s = store;
                const reports = s[repKey]?.dailyReports || [];
                return reports.length > 0 ? (
                  <div style={{ marginTop: 24 }}>
                    <h4 style={{ fontSize: 13, fontWeight: 700, marginBottom: 10, color: "var(--text2)" }}>
                      EXISTING REPORTS FOR {s[repKey]?.username?.toUpperCase()} ({reports.length})
                    </h4>
                    <div style={{ background: "var(--surface)", borderRadius: 12, border: "1px solid var(--border)", overflow: "hidden" }}>
                      ...
                      {reports.map((r, i) => (
                        <div key={i} style={{
                          display: "grid", gridTemplateColumns: "110px 90px 70px 80px 90px 90px 36px",
                          gap: 8, padding: "10px 14px", borderTop: "1px solid var(--border)",
                          fontSize: 12, fontFamily: "monospace", alignItems: "center",
                        }}>
                          <span style={{ color: "var(--text3)" }}>{r.date}</span>
                          <span style={{ color: r.profit >= 0 ? "#10d98a" : "#f43f5e", fontWeight: 700 }}>
                          ...
                          <button onClick={() => deleteReport(repKey, i)} style={{
                            background: "rgba(244,63,94,0.15)", border: "1px solid rgba(244,63,94,0.3)",
                            borderRadius: 6, color: "#f43f5e", cursor: "pointer", fontSize: 13, padding: "2px 6px",
                          }}>✕</button>
                        </div>
                      ))}
                    </div>
          ...
          {tab === "discord" && (
            <div>
              ...
              <p style={{ fontSize: 12, color: "var(--text3)", marginBottom: 20, lineHeight: 1.7 }}>
                Simulate <code style={{ color: "var(--accent)" }}>/running (key)</code> and{" "}
                <code style={{ color: "var(--accent)" }}>/stop (key)</code> here.
                Your real Discord bot should update the <code>connected</code> field in the database when those slash commands are called.
              </p>

              <Input label="USER KEY" value={discordKey} onChange={setDiscordKey} placeholder="USER-KEY-HERE" mono />
              <Select label="COMMAND" value={discordAction} onChange={setDiscordAction}
                options={[
                  { id: "running", label: "🟢 /running — Mark bot as Connected" },
                  { id: "stop",    label: "🔴 /stop — Mark bot as Offline" },
                ]} />
                ...
                {setupAccounts.map(acc => (
                  <div key={acc.key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      <Btn size="sm" variant={acc.connected ? "ghost" : "success"} onClick={async () => {
                        await update(ref(db, `accounts/${acc.key}`), { connected: true });
                        await refresh();
                        flash(`${acc.username} → CONNECTED`);
                      }}>Connect</Btn>
                      <Btn size="sm" variant={!acc.connected ? "ghost" : "danger"} onClick={async () => {
                        await update(ref(db, `accounts/${acc.key}`), { connected: false });
                        await refresh();
                        flash(`${acc.username} → OFFLINE`);
                      }}>Stop</Btn>
                    </div>
                  </div>
                ))}
...
function Dashboard({ account: initAccount, keyVal, onLogout }) {
  ...
  // Real-time Firebase listener for this account
  useEffect(() => {
    const accountRef = ref(db, `accounts/${keyVal}`);
    const unsub = onValue(accountRef, (snap) => {
      if (snap.exists()) {
        const data = snap.val();
        setAccount(a => ({ ...a, connected: data.connected, stats: data.stats, dailyReports: data.dailyReports }));
      }
    });
    return () => unsub();
  }, [keyVal]);
  ...
  const saveSettings = async () => {
    setSending(true);
    await update(ref(db, `accounts/${keyVal}`), { settings });
  ...
  const toggleSite = (site, checked) => {
    setSettings(s => ({
      ...s,
      sites: checked ? [...s.sites, site] : s.sites.filter(x => x !== site),
    }));
  };
              ...
              <div style={{
                ...
                <div style={{ marginTop: 20 }}>
                  ...
                  {reports.slice(0, 5).length === 0 ? (
                    <div style={{ color: "var(--text3)", fontSize: 12 }}>No activity yet</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              ...
              <div>
                ...
                <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 20, marginBottom: 16 }}>
                  ...
                  {STRATEGIES.map(s => (
                    <div key={s.id} onClick={() => updateSetting("strategy", s.id)} style={{
                      padding: "12px 14px", borderRadius: 10, marginBottom: 8, cursor: "pointer",
                      border: `1px solid ${settings.strategy === s.id ? "var(--accent)" : "var(--border)"}`,
                      background: settings.strategy === s.id ? "rgba(99,102,241,0.12)" : "transparent",
                      transition: "all 0.2s",
                    }}>
                ...
                <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 20 }}>
                  ...
                  <Select label="CURRENCY" value={settings.currency} onChange={v => updateSetting("currency", v)}
                    options={["USD","EUR","GBP","AUD","CAD","BTC"].map(c => ({ id: c, label: c }))} />
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 20, marginBottom: 16 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    {SUPPORTED_SITES.map(s => (
                      <SiteCheckbox key={s} site={s} checked={settings.sites.includes(s)} onChange={v => toggleSite(s, v)} />
                    ))}
                  </div>
                </div>
                ...
                <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 20, marginBottom: 16 }}>
                  ...
                  <Input label="USERNAME" value={account.username} readonly />
                  <Input label="WEBSITE" value={account.website} readonly />
                  <Input label="ACCESS KEY" value={keyVal} readonly mono />
                </div>

                {/* Save */}
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  <Btn onClick={saveSettings} disabled={sending} fullWidth>
                    {sending ? "⟳ Sending..." : saved ? "✓ Settings Saved & Sent to Discord" : "💾 Save Settings & Notify Admin"}
                  </Btn>
                </div>
              </div>
            </div>
          ...
          <div style={{ animation: "fadeUp 0.4s ease" }}>
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden" }}>
              ...
              <div style={{
                ...
                background: "var(--bg2)",
                fontSize: 10, color: "var(--text3)", letterSpacing: "0.1em", fontWeight: 600,
              }}>
                <span>DATE</span><span>PROFIT</span><span>BETS</span><span>WIN RATE</span><span>VOLUME</span><span>SITE</span>
              </div>
              {reports.length === 0 ? (
                <div style={{ padding: 40, textAlign: "center", color: "var(--text3)", fontSize: 13 }}>
                  No reports yet. Your admin will add daily reports here.
                </div>
                    ...
                    {(() => {
                      ...
                      return (
                        <>
                          ...
                          {profits.map((v, i) => (
                            <circle key={i} cx={toX(i)} cy={toY(v)} r={3} fill={v >= 0 ? "#10d98a" : "#f43f5e"} />
                          ))}
                        </>
                      );
                    })()}
              ...
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 20 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 40 }}>
                  <svg viewBox="0 0 100 100" width="120" height="120">
                    {(() => {
                      const total = stats.wonBets + stats.lostBets || 1;
                      const wr = stats.wonBets / total;
                      const r = 40, cx = 50, cy = 50;
                      const angle = wr * Math.PI * 2;
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "var(--text3)" }}>
                      Avg odds: {stats.avgOdds.toFixed(2)}<br/>
...
export default function App() {
  const [screen, setScreen] = useState("key"); // key | setup | dashboard | admin
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", bottom: 20, right: 20, zIndex: 200 }}>
        {!showAdminLogin ? (
          ...
          <div style={{ display: "flex", gap: 8, background: "var(--surface)", padding: 8, borderRadius: 12, border: "1px solid var(--border)", animation: "fadeUp 0.2s ease" }}>
            <input
              autoFocus type="password" value={adminInput} onChange={e => setAdminInput(e.target.value)}
      ...
      {screen === "dashboard" && account && (
        <Dashboard account={account} keyVal={currentKey} onLogout={handleLogout} />
      )}
    </>
  );
}
